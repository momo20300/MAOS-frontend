...
export function PurchaseInvoiceForm({
  ...
  const [orderItems, setOrderItems] = React.useState<PINVItem[]>([
  ...
  const updateItem = (index: number, field: keyof PINVItem, value: string | number) => {
    setOrderItems(prev => {
      const updated = [...prev];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSubmit({ supplier, due_date: dueDate, items: orderItems.filter(i => i.item_code) });
      onOpenChange(false);
      setSupplier("");
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" /> Nouvelle Facture d'Achat
            </DialogTitle>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading || !supplier || orderItems.some(i => !i.item_code)} className="rounded-xl">
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Creer la facture
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
