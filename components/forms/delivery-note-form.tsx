...
import { Loader2, Truck, User, Calendar, Trash2, Plus } from "lucide-react";
...
export function DeliveryNoteForm({
  open, onOpenChange, onSubmit, customers, items: availableItems,
  ...
  const [orderItems, setOrderItems] = React.useState<DNItem[]>([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!customer || orderItems.some(i => !i.item_code)) return;
    setLoading(true);
    try {
      await onSubmit({ customer, posting_date: postingDate, items: orderItems.filter(i => i.item_code) });
      onOpenChange(false);
      setCustomer("");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DocumentHeader title="Bon de Livraison (DN)" />
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Truck className="h-5 w-5" /> Nouveau Bon de Livraison
            </DialogTitle>
            <DialogDescription>Selectionnez un client et ajoutez les articles a livrer.</DialogDescription>
          </DialogHeader>

          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label className="flex items-center gap-2"><User className="h-4 w-4 text-muted-foreground" /> Client *</Label>
                <Select value={customer} onValueChange={setCustomer}>
                  <SelectTrigger className="rounded-xl"><SelectValue placeholder="Selectionnez un client" /></SelectTrigger>
                  <SelectContent>
                    {customers.map(c => <SelectItem key={c.name} value={c.name}>{c.customer_name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="grid gap-2">
                <Label className="flex items-center gap-2"><Calendar className="h-4 w-4 text-muted-foreground" /> Date de livraison</Label>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading || !customer || orderItems.some(i => !i.item_code)} className="rounded-xl">
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Creer le bon de livraison
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
