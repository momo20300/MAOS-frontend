...
export default function QuotationsPage() {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [search, statusFilter, quotations, dateFrom, dateTo]);

  const { sortedData, sortKey, sortDir, toggleSort } = useSortableData(
    filteredQuotations as unknown as Record<string, unknown>[],
    "transaction_date",
    "desc"
  );
  ...
  const paginatedQuotations = sortedData.slice(startIndex, startIndex + pageSize) as unknown as Quotation[];
  ...
  if (loading) return <PageSkeleton title="Devis" kpiCount={3} />;
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      {/* Error */}
      {error && !quotations.length && (
        <ErrorMessage message={error} onRetry={fetchQuotations} />
      )}
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          <CardTitle>Liste des devis ({sortedData.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {/* Sort header bar */}
          <div className="hidden md:flex items-center gap-4 px-4 pb-2 border-b text-xs font-medium text-muted-foreground">
            ...
            <SortableHeader label="Client" sortKey="party_name" active={sortKey === "party_name"} direction={sortDir} onClick={toggleSort} className="flex-1" />
            ...
            <SortableHeader label="Validite" sortKey="valid_till" active={sortKey === "valid_till"} direction={sortDir} onClick={toggleSort} className="w-24" />
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      printDocumentPDF('Quotation', quotation.name)
